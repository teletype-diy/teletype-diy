...
const {Errors, FollowState} = require('@teletype-diy/teletype-diy-client')
...
class GuestPortalBinding {
  constructor ({client, portalId, remotePeerId, workspace, notificationManager, didDispose}) {
    this.client = client
  ...
  async initialize () {
    try {
      this.portal = await this.client.joinPortal(this.portalId, this.remotePeerId)
