...
class PortalBindingManager {
  ...
  createGuestPortalBinding (portalId, remotePeerId) {
    ...
    if (promise) {
      ...
      promise = this._createGuestPortalBinding(portalId, remotePeerId)
      promise.then((binding) => {
  ...
  async _createGuestPortalBinding (portalId, remotePeerId) {
    console.log(remotePeerId);
  ...
  async getRemoteEditorForURI (uri) {
    console.log(`trying to reach editor with uri: ${uri}`);
    const uriComponents = uri.replace('atom://teletype/', '').split('/')

    // TODO: here was a very basic validation for the portalId. is that worth adding back in?
