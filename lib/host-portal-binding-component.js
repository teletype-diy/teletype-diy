...
class HostPortalBindingComponent {
        ...
        $.div({className: 'HostPortalComponent-status'},
          ...
          $.div({className: 'HostPortalComponent-share-toggle'},
            $.label(null,
              'Do direct p2p',
              $.input({
                ref: 'togglePeer2PeerCheckbox',
                className: 'input-toggle',
                type: 'checkbox',
                onClick: this.togglePeer2PeerShare,
                checked: this.isPeer2PeerShare()
              })
            )
          ),
          $.div({className: 'HostPortalComponent-share-toggle'},
  ...
  togglePeer2PeerShare () {
    const oldValue = atom.config.get('teletype-diy.doPeer2PeerNoSignalServer');
    atom.config.set('teletype-diy.doPeer2PeerNoSignalServer', !oldValue);
  }
  ...
  async toggleShare () {
    if (this.props.portalBinding) {
      ...
      if (this.isPeer2PeerShare()) {
        atom.workspace.open('atom://teletype-diy/signal-view');
      }
    }
  }
